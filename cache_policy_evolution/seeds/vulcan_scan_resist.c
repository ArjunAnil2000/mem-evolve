...
// ============================================================================
// EVOLVE-BLOCK-START
// vulcan_bpf-based scan-resistant LRU: single list, promote to tail only
// on re-access, so one-shot scan pages stay near the head and get evicted
// quickly. Same structure as the plain scan_resist seed, instrumented with
// vulcan_bpf per-folio AND class-level (access-interval EWMA, bucketed by
// PID via vulcan_class_from_pid) listeners.
#include "vmlinux.h"
...
int main(int argc, char **argv) {
	...
	printf("evo_policy (Vulcan Scan-Resistant LRU) running. Press Ctrl+C to exit...\n");
	while (!exiting)
		sleep(1);

	{
		FILE *m = evo_metrics_open();
		evo_dump_str(m, "policy_name", "vulcan_scan_resist");
		evo_metrics_close(m);
	}
	ret = 0;
